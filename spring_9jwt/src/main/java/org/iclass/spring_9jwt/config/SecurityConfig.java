...
public class SecurityConfig {
	...
	@Value("${CORS_ALLOWED_ORIGIN:localhost}") // 실습 3. 환경변수 cors 값 가져오기
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests(authorize -> authorize
				.requestMatchers(HttpMethod.OPTIONS, "/**").permitAll() // 😏preflight 를 위한 OPTIONS 메소드 허용하기(반드시 가장 먼저.)
	...
	CorsConfigurationSource corsConfigurationSource() {
		...
		// 프론트 주소 cors 설정 : 실습 1,2
		// config.setAllowedOrigins(List.of("http://localhost",
		// "http://localhost:5173")); // 실습2에서 http://localhost 추가
		// 실습 3. 환경 변수에서 가져오기
